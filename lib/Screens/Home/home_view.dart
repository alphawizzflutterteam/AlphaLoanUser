...
import 'package:eggrow/Screens/Home/add_product_screen.dart';
 ...
 @override
  ...
  openwhatsapp() async {
   ...
   // if (await canLaunch(whatsappURl_android)) {
    try{
      await launchUrl(Uri.parse(whatsappURl_android),mode: LaunchMode.externalApplication);
  ...
  //          // String whatsappUrl = "https://wa.me/$encodedPhoneNumber?text=$encodedMessage";
  //       return "https://wa.me/9926202390?text=hii";
  //     } else {
  //       return "https://wa.me/9926202390/?text=hii";
  //     }
  //   }
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    DateTime currentDate = lastDate.subtract(Duration(days: 30));
...
// Assuming picked is of type TimeOfDay
        selectedDate=picked;
        DateTime now = DateTime.now();
        DateTime dateTime = DateTime(selectedDate.year, selectedDate.month, selectedDate.day, selectedDate.hour, selectedDate.minute);
        selectedDate1 = DateFormat("dd MMM yyyy").format(dateTime);
        filterDate=DateFormat("dd MMM, yyyy").format(dateTime);
        eggData=[];
        eggsModel?.data?.forEach((element) {
          if(filterDate==element.date){
            isAvailabel=true;
            eggData.add(element);
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        //       ),
        //       child: Center(child: Text("Add",style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontSize: 20),)),
        //     ),
        //   ),
        // ),
          key: _key,
          floatingActionButton: Column(
            ...
            children: [
              ...
              //   backgroundColor: Color(0xffCB100A),
        ...
        appBar: AppBar(
              ...
              child: InkWell(
                onTap: () {
                 Navigator.push(context, MaterialPageRoute(builder: (context)=> NotificationScreen(),
                  ...
                  child: Image.asset("assets/images/notification.png", height: 18, width:20,color: AppColors.whit)),
            ),
          ],
          ...
          body: eggsModel == null || _isLoading==true ? Center(child: CircularProgressIndicator()):
          filterDate=="" || (filterDate!=""  && eggData.length > 0) ? ListView.builder(
              itemCount: filterDate!="" ? eggData.length : eggsModel?.data?.length,
              shrinkWrap: true,
              itemBuilder: (c, i){
                // print(eggsModel!.data![i].date);
                // print(filterDate);
                // print(eggsModel!.data![i].date == filterDate);
                List  newDatalist =  neewData("${eggsModel?.data?[i].location}");
                List  newDatalistprice =  neewData("${eggsModel?.data?[i].price}");
                return filterDate!="" ? Padding(
                            ...
                            children: [
                              Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    //  Text(DateFormat("dd MMM yyyy").format(DateTime.parse(eggsModel!.data![i].date ?? "")), style: TextStyle(fontSize: 13, color: Colors.black, fontWeight: FontWeight.bold)),
                                    Row(
                                      children: [
                                        Icon(Icons.calendar_month,color: Colors.black,size: 19,),
                                        SizedBox(width: 5,),
                                        Text(eggData[i].date ?? "", style: TextStyle(fontSize: 15, color: Colors.black, fontWeight: FontWeight.bold)),
                                      ],
                                    ),
                                    Row(
                                      children: [
                                        ...
                                        SizedBox(width: 5,),
                                        Text("${eggData[i].time}", style: TextStyle(fontSize: 15, color: Colors.black, fontWeight: FontWeight.bold)),
                                      ],
                                    ),
                                  ],
                                ),
                              ...
                              for(int j=0;j<newDatalist.length;j++)...[
                                Padding(
                                  ...
                                  child: Row(
                                    //mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      Text("${newDatalist[j]} : ",style: TextStyle(fontSize: 15, color: Colors.black, fontWeight: FontWeight.bold)),
                                      //SizedBox(width: 10,),
                                      Text(" ${newDatalistprice[j]}",style: TextStyle(fontSize: 15, color:  Colors.black, fontWeight: FontWeight.bold)),
                                      // SizedBox(width: 10,),
                              ...
                              if(eggData[i].url !='')
                                InkWell(
                                  onTap: () {
                                    _launchURL("${eggData[i].url}");
                                  },
                                  child: Container(
                                      height: 30,
                                      width: double.infinity,
                                      decoration: const BoxDecoration(color: Color(0xfffFFE183),
                                          borderRadius: BorderRadius.only(bottomLeft: Radius.circular(10),bottomRight: Radius.circular(10))
                                      ),
                                      child: Padding(
                                        padding: const EdgeInsets.symmetric(horizontal: 10,vertical: 5),
                                        child: Text("${eggData[i].url}", style: TextStyle(fontSize: 15, color: Colors.blue, fontWeight: FontWeight.bold, )),
                                      )),
                                ),
                              //SizedBox(height: 8,)
                            ],
                              ...
                              Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    //     Icon(Icons.calendar_month,color: Colors.black,size: 19,),
                                    //     SizedBox(width: 5,),
                                    //     Text(eggsModel!.data![i].date ?? "", style: TextStyle(fontSize: 15, color: Colors.black, fontWeight: FontWeight.bold)),
                                    //   ],
                                    // ),
                                    // Row(
                                    //   children: [
                                    //     Icon(Icons.access_time,color: Colors.black,size: 19,),
                                    //     SizedBox(width: 5,),
                                    //     Text("${eggsModel?.data?[i].time}", style: TextStyle(fontSize: 15, color: Colors.black, fontWeight: FontWeight.bold)),
                                    //   ],
                                    // ),
                                  ],
                                ),
                              ...
                              if(eggsModel?.data?[i].url !='')
                                InkWell(
                                  ...
                                  child: Container(
                                      height: 30,
                                      width: double.infinity,
                                      decoration: BoxDecoration(color: Color(0xfffFFE183),
                                          borderRadius: BorderRadius.only(bottomLeft: Radius.circular(10),bottomRight: Radius.circular(10))
                                      ),
                                      child: Padding(
                                        padding: const EdgeInsets.symmetric(horizontal: 10,vertical: 5),
                                        child: Text("${eggsModel?.data?[i].url}", style: TextStyle(fontSize: 15, color: Colors.blue, fontWeight: FontWeight.bold, )),
                                      )),
                                ),
                              //SizedBox(height: 8,)
                            ],
                ...
                //  return eggsModel!.data![i].date == filterDate ?  Padding(
                //   padding:  EdgeInsets.only(left: 10, right: 10, top: 12),
                //   child: Card(
                //
                //     shape:  RoundedRectangleBorder(
                ...
                //                     children: [
                //                       //  Text(DateFormat("dd MMM yyyy").format(DateTime.parse(eggsModel!.data![i].date ?? "")), style: TextStyle(fontSize: 13, color: Colors.black, fontWeight: FontWeight.bold)),
                //                       Text(eggsModel!.data![i].date ?? "", style: TextStyle(fontSize: 13, color: Colors.black, fontWeight: FontWeight.bold)),
                //                       Text("${eggsModel?.data?[i].time}", style: TextStyle(fontSize: 13, color: Colors.black, fontWeight: FontWeight.bold)),
                //                     ],
                //                   ),
                //                 ),
                ...
                //                       mainAxisAlignment: MainAxisAlignment.spaceBetween,
                //                       children: [
                //                         Text("${newDatalist[j]}:",style: TextStyle(fontSize: 13, color: Color(0xff8e8e8e), fontWeight: FontWeight.bold)),
                //
                //                         Text(" ${newDatalistprice[j]}",style: TextStyle(fontSize: 13, color: Color(0xff8e8e8e), fontWeight: FontWeight.bold)),
                //                         // SizedBox(width: 10,),
                //                         // Text("${newDatalistprice[j]}")
                //                       ],
                //                     ),
            ...
            return  Column(
              ...
              children: [
                const Padding(
                  ...
                  child: Text('Welcome To Alpha Loan',style: TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                      color: Colors.black
                  ),),
...
// }
  ...
  eggsDetails() async {
    ...
    var request = http.MultipartRequest('POST', Uri.parse('https://alphaloan.productsalphawizz.com/Apicontroller/home_data'));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      var result = await response.stream.bytesToString();
